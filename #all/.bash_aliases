...
func_alias gitlf='git log -n 1 --format=%B'
...
func_alias gitfx='__() { [ -n "$1" ] || echo "Error: no commit" >&2 && return 1; local r="$(git rev-parse "$1")"; git commit --fixup "$r"; git rebase -i --autosquash "${r}~1"; }; __'
...
func_alias tmuxh='[ -n "$SCRIPT_FILE" ] && less -R $SCRIPT_FILE || echo "Error: No script file. Are you in a tmux session?"'
